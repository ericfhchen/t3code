...
describe("mobileThemeRuntime", () => {
  ...
  it("maps custom palettes and appearances to registered themes", () => {
    ...
    expect(getMobileUniwindThemeName("studio", "light")).toBe("studio-dark");
  });
  ...
  it("hydrates text variables and clears the native appearance override", () => {
    ...
    expect(variableOperations).toHaveLength(13);
    expect(variableOperations.at(-1)?.themeName).toBe("studio-dark");

...
it("does not activate T3 until requested, then matches PID and title", async () => {
  ...
  windows = [
    { ...window, id: 1, pid: 999, title: "CODE" },
    { ...window, id: 2, pid: process.pid, title: "Other T3" },
    { ...window, id: 3, pid: process.pid, title: "CODE" },
  ];
  await snapshot.feedback!.activate("CODE");
...
it("waits for the restored T3 window to map instead of polling", async () => {
  ...
  handler = async (request, socket) => {
    ...
    if (request === "EventStream")
      send(socket, {
        WindowOpenedOrChanged: { window: { ...window, id: 4, pid: process.pid, title: "CODE" } },
      });
  };
  await snapshot.feedback!.activate("CODE");
...
it("rejects ambiguous activation targets", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  windows = [1, 2].map((id) => ({ ...window, id, pid: process.pid, title: "CODE" }));
  await expect(snapshot.feedback!.activate("CODE")).rejects.toThrow("More than one");
});
...
it("cancels pending activation when capture feedback is closed", async () => {
  ...
  const activation = expect(snapshot.feedback!.activate("CODE")).rejects.toThrow("cancelled");
